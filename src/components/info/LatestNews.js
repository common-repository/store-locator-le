...
const LatestNews = () => {
    ...
    useEffect(() => {
        function getData() {
            ...
            axios.get(`${slpReact.url.main_site}/wp-json/wp/v2/posts/?_fields=title,date,excerpt,jetpack_featured_media_url,link&per_page=5`)
                // -- response received
    ...
    return (
        <>
            ...
            <Card raised loading={loading}>
                <CardHeader title={__('News', 'store-locator-le')}/>
                <CardContent>
                    <Stack spacing={1}>
