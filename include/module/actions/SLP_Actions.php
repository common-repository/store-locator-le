...
class SLP_Actions extends SLPlus_BaseClass_Object {
	...
	public function save_screen_options( $status, $option, $value ) {
		switch ( $this->slplus->clean['page'] ) {
			case 'slp_manage_locations':
				return SLP_Admin_Locations::get_instance()->save_screen_options( $status, $option, $value );
				break;
		}
	...
	public function init() {
		...
		//
		add_action( 'render_block_context', array( $this, 'wp_enqueue_scripts_for_shitty_block_themes' ) );
	 ...
	 * @return mixed
	 */
	public function wp_enqueue_scripts_for_shitty_block_themes( $context, $parsed = [], $parent = null ) {
	...
	function wp_head() {
		...
		echo '<!-- SLP Custom CSS -->' . "\n" . '<style type="text/css">' . "\n" .
		     ...
		     // Map
		     "div#map.slp_map {\n" .
		     esc_attr( "width:{$this->slplus->options_nojs['map_width']}{$this->slplus->options_nojs['map_width_units']};\n" ) .
		     esc_attr( "height:{$this->slplus->options_nojs['map_height']}{$this->slplus->options_nojs['map_height_units']};\n" ) .
		     "}\n" .
		     ...
		     //
		     esc_attr( apply_filters( 'slp_ui_headers', '' ) ) .
	...
	public function network_admin_menu() {
		add_menu_page(
			...
			'slp-network-admin',
			array( SLP_Admin_General::get_instance(), 'display' ),
			SLPlus::menu_icon
		);
	}
