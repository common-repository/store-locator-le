...
class SLP_Text_Links extends SLPlus_BaseClass_Object {
	...
	private function build_dynamic_link( $slug, $option ) {
		...
		if ( empty( $link_text ) ) {
			$link_text = $this->slplus->Text->get_text_string( array( 'general', $option ), false );
		}
	...
	private function docs_for( $slug ) {
		...
		$this->web_links[ $slug ] = new SLP_Web_Link( array(
			...
			'link_text' => $link_text,
			'url'       => $this->get_url( 'docs_search' ) . str_replace( '_', '-', $feature ),
		) );
	}
	...
	private function icon_for( $slug ) {
		...
		$original_link_text = "<div class='sprite_48 {$icon_class}'></div>";
		$link_text          = $icon_class;
	...
	private function init_urls() {
		...
		$this->urls['docs_search']            = $this->urls['slp_docs'] . 'documentation/tag/';
		...
		$this->urls['upgrade_wp_version'] = $this->urls['docs_tag'] . 'updates/';
