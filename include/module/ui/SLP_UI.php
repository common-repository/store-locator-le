...
class SLP_UI extends SLPlus_BaseClass_Object {
	...
	public function modify_searchlayout( $html ) {
		...
		// Hide Address Input
		//
		if ( $this->slplus->SmartOptions->is_true( 'hide_address_entry' ) ) {
	...
	public function create_string_radius_selector_div() {
		if ( $this->slplus->SmartOptions->is_true( 'hide_radius_selector' ) ) {
	...
	public function create_Map() {
		...
		// Remove the credits
		//
		if ( $this->slplus->SmartOptions->is_true( 'remove_credits' ) ) {
	...
	public function render_shortcode( $attributes, $content = null ) {
		...
		if ( ! empty( $attributes ) ) {
			 ...
			 *
			 * param1 - array list of allowed attributes and their defaults
			 * param2 - array the attribute key=>value pairs from the shortcode being processed [slplus att='val']
			 * param3 - array content between the start and end shortcode block, always empty for slplus.
			 */
	...
	private function set_allowed_attributes( $attributes ) {
		...
		foreach ( $smart_options as $option ) {
			if ( 'checkbox' !== $this->slplus->SmartOptions->get_TypeOf( $option ) ) {
				$allowed_attributes[ $option ] = $this->slplus->SmartOptions->get_ValueOf( $option );
			} else {
				$allowed_attributes[ $option ] = $this->slplus->SmartOptions->is_true( $option ) ? '1' : '0';
			}
		}
	...
	public function add_to_js_options( $options ) {
		return array_merge( $options, array(
			...
			'use_sensor'    => SLP_SmartOptions::get_instance()->is_true( 'use_sensor' )
		) );
	}
