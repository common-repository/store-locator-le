 ...
 * @property        mixed[] $attributes             The deserialized option_value field. This can be augmented by multiple add-on packs. Tagalong adds: array[] ['store_categories'] int[] ['stores']
 *
 * PAGE RELATIONS:
...
class SLPlus_Location {
	...
	private function set_PageData() {
		...
		// pageData.tax_input.stores has the array of category ids when it gets here
	...
	function isvalid_ID( $id = null, $request_param = null ) {
		if ( ! empty( $_REQUEST['security'] ) && ! check_admin_referer( 'slp_ajax', 'security' ) ) {
			return false;
		}
