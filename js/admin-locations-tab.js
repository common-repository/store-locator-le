/*global location_manager, Vue, Vuetify */
/**
...
SLP_Location_Manager.vue = (function () {
    ...
    this.initialize = function () {
        ...
        this.load_app = new Vue({
            ...
            methods: {
                ...
                load_locations: function () {
                    var _this = this,
                        full_url =
                            (_this.boolean.site_import_protocol ? 'https://' : 'http://') +
                            _this.site_import_url +
                            '/wp-json/store-locator-plus/v2/locations';
                    ...
                    fetch(full_url)
                        .then(function (list_response) {
                            list_response.json().then(function (location_list) {
                                location_list.forEach(function (location_data) {
                                    fetch_location(location_data.sl_id).then(function (location) {
                ...
                add_location: function (location) {
                    ...
                    jQuery.ajax({
                        ...
                        error: function (error_response) {
                            var msg = error_response.responseText
        ...
        this.update_app = new Vue({
            ...
            methods: {
                close_add_dialog: function (event) {
                    SLP_Location_Manager.Editor.close();
                },
                submit_form: function () {
                    AdminUI.doAction(this.act);
                },
            },
        });
    };
...
var SLP_Location_Form_Handler = function () {
    ...
    this.populate_field = function (field, source, destination) {
        ...
        this.set_value(
            destination.find(field_finder),
            this.get_value(source.find(field_finder))
        );
        return field;
    };
...
var SLP_Locations_map = function () {
    ...
    this.show_map = function (location_element) {
        ...
        lat = SLP_Location_Manager.Form_Handler.get_value(
            location_element.find('[data-field="sl_latitude"]')
        );
        lng = SLP_Location_Manager.Form_Handler.get_value(
            location_element.find('[data-field="sl_longitude"]')
        );
        if (!lat) return;
        if (!lng) return;
        ...
        //
        slp_Admin_Filter('location_map_initialized').publish();
    };
    ...
    this.set_icon = function (location_element) {
        ...
        if (!icon) {
            icon = location_element
                .find('[data-field="category_marker"]')
                .attr('src');
            if (!icon) {
...
var SLP_Locations_table = function () {
     ...
     */
    this.initialize = function () {
        jQuery('a[data-action="delete"]').on('click', this.delete_location);
        jQuery('td[data-field="sl_description"]').on(
            'click',
    ...
    this.delete_location = function (event) {
        ...
        slp_Admin_Filter('ajax_post_ok').subscribe(function (response) {
            tr.remove();
        });

        // Do this when delete did not work (backend).
        slp_Admin_Filter('ajax_post_info').subscribe(function (response) {
            tr.fadeIn(200);
        });

        // Do this when delete did not work (service AWOL).
        slp_Admin_Filter('ajax_post_failure').subscribe(function (response) {
            tr.fadeIn(200);
        });
...
var SLP_Locations_table_header = function () {
     ...
     */
    this.initialize = function () {
        jQuery('#do_action_apply_to_all').on('click', this.execute_apply_to_all);
        jQuery('#do_action_apply').on('click', this.execute_apply);
    };
    ...
    this.execute_apply_bulk_action = function (all) {
        ...
        if (confirm('Are you sure you want to ' + action_text + '?')) {
            ...
            // -- name the filter <action>BulkAction> , i.e. exportBulkAction
            const slpFilterName = action_val + 'BulkAction';

            // Fire custom filters if defined
            if (slp_Admin_Filter(slpFilterName).hasSubscriptions()) {
                slp_Admin_Filter(slpFilterName).publish();

                // otherwise fire the form submit by default
            } else {
...
var SLP_Location_Card = function () {
     ...
     */
    this.initialize = function () {
        jQuery('span.store_name').on('click', function () {
            SLP_Location_Manager.Card.show_location_details(
                jQuery(this).parents('tr').first()
            );
        });
        jQuery('.location-card').on('click', function () {
    ...
    this.show_location_details = function (table_row) {
        ...
        already_displayed.push(
            SLP_Location_Manager.Form_Handler.populate_field(
                'sl_store',
                table_row,
                header
            )
        ...
        for (var cnt = 0, len = location_cells.length; cnt < len; cnt++) {
            ...
            card_grid.append(
                ...
                label +
                '</strong>' +
                '<span contenteditable="true" data-field="' +
                field +
                '">' +
                location_cells[cnt].innerHTML +
                '</span>' +
                '</div>'
            );
        }
        ...
        var card_right =
            jQuery(window).width() - (table_position.left + table_width);
...
var SLP_Location_Editor = function () {
    ...
    this.initialize = function () {
        ...
        jQuery('#wpcsl-option-add_sidemenu').on('click', function () {
            SLP_Location_Manager.Editor.load_and_show_form(jQuery(this));
        });
    ...
    this.close = function () {
        ...
        jQuery('#wpcsl-option-current_locations_sidemenu').trigger('click');
    };
    ...
    this.load_and_show_form = function (action_button) {
        ...
        SLP_Location_Manager.Form_Handler.hide(jQuery('.location-card'));
        jQuery('#myslp-header').on('click', this.close);
        jQuery('#dashboard-header').on('click', this.close);
...
jQuery(document).ready(function () {
    ...
    //
    if (
        jQuery('#wpcsl-option-current_locations div.section_description').is(
            ':empty'
        )
    ) {
        jQuery('#wpcsl-option-add_location_sidemenu').trigger('click');
    }
